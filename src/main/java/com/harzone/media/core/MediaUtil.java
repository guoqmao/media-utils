...
import com.harzone.media.core.domain.ImageMetaInfo;
import com.harzone.media.core.domain.MusicMetaInfo;
import com.harzone.media.core.domain.VideoMetaInfo;
import com.harzone.media.core.domain.gif.AnimatedGifEncoder;
...
public class MediaUtil {
	...
	public static String executeCommand(List<String> commands, File envPath) {
		...
		try {
			...
			if (null != envPath){
			    builder.directory(envPath);
            }
			ffmpeg = builder.start();
			log.info("--- 开始执行FFmpeg指令[{}]，工作目录为：[{}]",
                    String.join(" ", ffmpegCmds), envPath);

            // 取出输出流和错误流的信息
            // 注意：必须要取出ffmpeg在执行命令过程中产生的输出信息，
            // 如果不取的话当输出流信息填满jvm存储输出留信息的缓冲区时，线程就会阻塞住
            ...
            String resultStr = StringUtils.isNotBlank(errorStream.stringBuffer) ? "【异常】" : "正常";
